...
let AuthCookie = require('hapi-auth-cookie');
let Yar = require('yar');
...
let CompanyDropdownButton = React.createFactory(require('./lib/components/CompanyDropdownButton'));
...
let QuickBooks = require('./lib/utils/QBO');
let createQBO = QuickBooks.init(CONSUMER_KEY, CONSUMER_SECRET, /*useSandbox*/SANDBOX, /*useDebug*/true);
              ...
              db.find('Company', {
                ...
                function(findErr, companies) {
                  if (findErr) {
                    ...
                    let filteredCompanies = filterCompanies(companies.results, SANDBOX);
                    let session = {
                      user: {
                        id: user.objectId,
                        email: user.email,
